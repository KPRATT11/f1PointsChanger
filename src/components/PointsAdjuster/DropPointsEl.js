...
const DropPointsEl = (props) => {
  const [dropScores, setDropScores] = useState([
    props.dropPoints[0],
    props.dropPoints[1],
  ]);
  ...
  const changeAmountKept = (e) => {
    if (e.target.value <= dropScores[1]) {
      ...
      console.log(
        "Cant have amount of races to drop be lower than races dropped"
      );
    }
  };
  ...
  return (
    <div>
      <Input
        ...
        value={dropScores[0]}
        onChange={(val) => {
          updateDropScores(val.target.value, 0);
        }}
      />
      ...
      <Input
        ...
        value={dropScores[1]}
        onChange={(val) => {
          updateDropScores(val.target.value, 1);
        }}
      />
    </div>
  );
